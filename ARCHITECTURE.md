...
codexforge is a thin, opinionated control plane wrapped around the Claude Agent SDK runtime. Its job is to compose specialist subagents into repeatable maintainer workflows while keeping every write action behind human approval.
...
Persists session IDs, metadata, and the audit log to SQLite. Enables `codexforge resume <session>` semantics across CLI runs.
...
codexforge composes at least two MCP servers in production:
