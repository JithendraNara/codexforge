...
codexforge evaluates three qualities: **routing correctness**, **safety gating**, and **output structure**. The eval runs without network access by replaying canned scenarios through the orchestrator.
...
GitHub Actions runs `codexforge eval` on every push. Failing metrics block merges.
