"""Environment-driven configuration for codexforge.
...
def _resolve_approval_mode(raw: str | None) -> ApprovalMode:
    ...
    raise ValueError(
        f"Invalid CODEXFORGE_APPROVAL_MODE value: {raw!r} (expected 'auto' or 'manual')"
    )


def load_config() -> CodexForgeConfig:
    """Build a :class:`CodexForgeConfig` from the current environment."""

    data_dir = _resolve_data_dir(os.environ.get("CODEXFORGE_DATA_DIR"))
    ...
    return CodexForgeConfig(
        ...
        github_token=os.environ.get("CODEXFORGE_GITHUB_TOKEN"),
        approval_mode=_resolve_approval_mode(os.environ.get("CODEXFORGE_APPROVAL_MODE")),
        data_dir=data_dir,
    )
