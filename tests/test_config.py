...
from codexforge.config import load_config
...
def test_load_config_uses_defaults(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    for key in [
        ...
        "ANTHROPIC_MODEL",
        "CODEXFORGE_GITHUB_TOKEN",
        "CODEXFORGE_APPROVAL_MODE",
    ]:
        monkeypatch.delenv(key, raising=False)
    monkeypatch.setenv("CODEXFORGE_DATA_DIR", str(tmp_path))
...
def test_load_config_with_env_overrides(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CODEXFORGE_APPROVAL_MODE", "auto")
    monkeypatch.setenv("CODEXFORGE_DATA_DIR", str(tmp_path))
...
def test_invalid_approval_mode_raises(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("CODEXFORGE_APPROVAL_MODE", "weird")
    monkeypatch.setenv("CODEXFORGE_DATA_DIR", str(tmp_path))
