"""MCP server exposing codexforge workflows to any Claude Agent SDK client.

This module registers four tools that call codexforge's agentic
workflows end-to-end so tools like Claude Desktop, Cursor, OpenCode,
and Windsurf can drive codexforge remotely when configured with the
MiniMax Anthropic-compatible endpoint.

The SDK import is deferred so the rest of codexforge stays importable
...
def build_server() -> Any:
    ...
    @tool(
        "codexforge_triage",
        "Run the codexforge agentic triage workflow against a GitHub issue.",
        {"repo": str, "issue": int},
    )
    ...
    @tool(
        ...
        "Run the codexforge investigation workflow over a local repository.",
    ...
    @tool(
        "codexforge_code",
        "Run the codexforge coding workflow for a targeted patch proposal.",
    ...
    @tool(
        "codexforge_review",
        "Run the codexforge PR review workflow against a GitHub pull request.",
        {"repo": str, "pr": int},
    )
...
def server_metadata() -> dict[str, Any]:
    ...
    return {
        ...
        "tools": [
            {
                "name": "codexforge_triage",
                "description": "Run the codexforge agentic triage workflow against a GitHub issue.",
                "parameters": {"repo": "str", "issue": "int"},
            },
            {
                "name": "codexforge_investigate",
                "description": "Run the codexforge investigation workflow over a local repository.",
                "parameters": {"repo_root": "str", "issue_title": "str", "issue_body": "str"},
            },
            {
                "name": "codexforge_code",
                "description": "Run the codexforge coding workflow for a targeted patch proposal.",
                "parameters": {
                    "repo_root": "str",
                    "target_file": "str",
                    "goal": "str",
            ...
            {
                "name": "codexforge_review",
                "description": "Run the codexforge PR review workflow against a GitHub pull request.",
