"""Subagent registry for codexforge.
...
TRIAGE_AGENT = SubagentSpec(
    ...
    system_prompt=(
        "You are codexforge's triage specialist. Read the issue or pull"
...
INVESTIGATOR_AGENT = SubagentSpec(
    ...
    system_prompt=(
        "You are codexforge's investigator. Build a reproduction plan, try"
...
CODER_AGENT = SubagentSpec(
    ...
    system_prompt=(
        "You are codexforge's coding specialist. Propose the smallest patch"
...
REVIEWER_AGENT = SubagentSpec(
    ...
    description=(
        ...
        " compatibility with the codexforge safety model."
    ),
    system_prompt=(
        "You are codexforge's reviewer. Critique the proposed change on"
...
SCRIBE_AGENT = SubagentSpec(
    ...
    system_prompt=(
        "You are codexforge's scribe. Write calm, accurate, maintainer-voice"
...
def get_subagent(name: str) -> SubagentSpec:
    try:
        ...
        raise KeyError(f"Unknown codexforge subagent: {name!r}") from exc
