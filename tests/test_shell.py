...
from codexforge.adapters.shell import CommandRefused, ShellAdapter, ShellPolicy
...
def test_policy_runs_allowlisted_command(tmp_path: Path) -> None:
    ...
    result = shell.run(["echo", "codexforge"], cwd=tmp_path)
    assert result.returncode == 0
    assert "codexforge" in result.stdout
