...
through MiniMax's Anthropic-compatible endpoint. codexforge is not a
...
- MCP server exposing codexforge workflows to any Claude Agent SDK
...
codexforge ships an MCP server (`src/codexforge/mcp_server.py`) with
four tools:

- `codexforge_triage(repo, issue)`
- `codexforge_investigate(repo_root, issue_title, issue_body)`
- `codexforge_code(repo_root, target_file, goal, evidence)`
- `codexforge_review(repo, pr)`

Any Claude Agent SDK host can mount the server via
`ClaudeAgentOptions(mcp_servers=...)` and delegate maintainer work to
codexforge through structured tool calls.

## Observability

`src/codexforge/telemetry/tracing.py` instruments the agent loop,
