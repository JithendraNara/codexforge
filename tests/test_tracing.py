...
from codexforge.telemetry.tracing import Tracer, configure


def test_configure_returns_tracer_without_otel(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CODEXFORGE_TRACING", "disabled")
    tracer = configure()
...
def test_noop_span_context_is_safe() -> None:
    tracer = Tracer(None)
    with tracer.span("codexforge.test", {"answer": 42}) as span:
